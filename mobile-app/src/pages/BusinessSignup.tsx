...
const BusinessSignup = () => {
  ...
  const isAgroSignup = signupType === 'agro';
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (userData.success) {
        ...
        // Navigate to login
        navigate(isAgroSignup ? '/agro-login' : '/login?type=business');
      }
    } catch (e: any) {
        ...
        <Card>
          <CardHeader className="text-center">
            <div className="flex justify-center mb-4">
              <div className={`h-12 w-12 rounded-lg flex items-center justify-center ${isAgroSignup ? 'bg-emerald-100 dark:bg-emerald-900' : 'bg-primary/10'}`}>
                {isAgroSignup ? (
            ...
            <CardTitle className="text-2xl">{isAgroSignup ? 'Create Farm Account' : 'Create Business Account'}</CardTitle>
            <CardDescription>
              {isAgroSignup ? 'Start managing your farm with smart tools' : 'Start managing your business with AI-powered tools'}
            </CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSignup} className="space-y-4">
              ...
              <Button
                type="submit"
                className={`w-full ${isAgroSignup ? 'bg-emerald-600 hover:bg-emerald-700' : ''}`}
                disabled={isLoading}
              >
                {isLoading ? (
                  <>
              ...
              <div className="text-center text-sm text-muted-foreground">
                ...
                <Button
                  ...
                  onClick={() => navigate(isAgroSignup ? '/agro-login' : '/login?type=business')}
                >
                  Sign in
                </Button>
              </div>
