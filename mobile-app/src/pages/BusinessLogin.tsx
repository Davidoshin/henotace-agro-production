...
const BusinessLogin = () => {
  ...
  const handleBiometricLogin = async () => {
    ...
    try {
      ...
      if (Capacitor.isNativePlatform()) {
        ...
        try {
          ...
          try {
            ...
            console.error('No stored credentials found:', credError);
            // Clear enrolled status since credentials don't exist
            localStorage.removeItem('biometrics_enrolled');
            localStorage.removeItem('biometrics_email');
            setBiometricsEnrolled(false);
            setError('Biometric credentials not found. Please enable biometric login from your account settings first.');
            setIsBiometricLoading(false);
            return;
          }
          ...
          if (!credentials || !credentials.username || !credentials.password) {
            ...
            localStorage.removeItem('biometrics_email');
            setBiometricsEnrolled(false);
            setError('Biometric credentials not configured. Please enable biometric login from your account settings.');
            setIsBiometricLoading(false);
            return;
          }

          // Now verify identity with biometrics
          await NativeBiometric.verifyIdentity({
            reason: 'Sign in to Henotace Business',
            title: 'Biometric Login',
            subtitle: 'Henotace Business',
            description: 'Use your fingerprint or face to sign in',
          });

          loginEmail = credentials.username;
          loginPassword = credentials.password;
        } catch (biometricError: any) {
          console.error('Native biometric error:', biometricError);
          if (biometricError.code === 10 || biometricError.code === 'ERROR_BIOMETRIC_AUTHENTICATION_FAILED') {
            setError('Biometric authentication failed. Please try again.');
          } else if (biometricError.code === 13 || biometricError.code === 16) {
            setError('Biometric authentication was cancelled.');
          } else if (biometricError.message?.includes('credentials') || biometricError.message?.includes('No credentials')) {
            localStorage.removeItem('biometrics_enrolled');
            localStorage.removeItem('biometrics_email');
            setBiometricsEnrolled(false);
            setError('Biometric credentials not found. Please enable biometric login from your account settings.');
          } else {
        ...
        loginPassword = atob(storedToken);
      }
      ...
      if (err.name === 'NotAllowedError') {
        ...
        setError(err.message || 'Biometric login failed. Please try email and password.');
      }
    } finally {
      setIsBiometricLoading(false);
    }
  ...
  const handleSuccessfulLogin = (data: any) => {
    // Store tokens - handle both nested (tokens.access) and flat (access) response formats
    ...
    localStorage.setItem('access_token', accessToken); // Also set for api.ts compatibility
  ...
  const handle2FAVerify = async () => {
    ...
    try {
      ...
      const response = await fetch(`${getBaseUrl()}auth/2fa/verify-login-session/`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
