...
export const DashboardLayout = () => {
  ...
  const handleLogout = async () => {
    // Use secure logout to properly invalidate server-side tokens
    // This clears HttpOnly cookies AND localStorage
    try {
      const { secureLogout } = await import('@/lib/api');
      await secureLogout();
    } catch (error) {
      console.error('Logout error:', error);
      // Fallback: clear localStorage manually if import fails
      localStorage.removeItem('accessToken');
      localStorage.removeItem('refreshToken');
      localStorage.removeItem('userRole');
      localStorage.removeItem('userData');
