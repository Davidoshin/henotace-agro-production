...
import { CreditCard, Building, Clock, Copy, RefreshCw, CheckCircle as CheckCircleIcon, AlertTriangle, Truck } from 'lucide-react';
...
export default function PaymentMethodDialog({
  ...
  const [paymentMethod, setPaymentMethod] = useState<'online' | 'bank_transfer' | 'pay_on_delivery'>('online');
  ...
  const loadPaymentInfo = async () => {
    ...
    try {
      ...
      // Get business payment settings for pay on delivery
      try {
        const businessData = await apiGet('business/info/') as any;
        if (businessData.success) {
          const enabled = businessData.pay_on_delivery_enabled || false;
  ...
  const handlePayOnDelivery = async () => {
    setProcessingPayment(true);
    try {
      // Call the business payment initiation endpoint with pay_on_delivery method
      const response = await apiPost('business/payment/initiate/', {
        amount,
        payment_method: 'pay_on_delivery',
        notes: 'Pay on delivery - payment to be collected when order is delivered'
      }) as any;

      if (response.success) {
        toast({
          title: 'Order Created',
          description: response.message || 'Your order has been created. You can pay when you receive your order.',
        });
      ...
      <DialogContent className="max-w-[95vw] sm:max-w-2xl max-h-[90vh] p-0">
        ...
        <ScrollArea className="max-h-[60vh] px-6">
          <div className="py-4">
            ...
            <RadioGroup value={paymentMethod} onValueChange={(value) => setPaymentMethod(value as 'online' | 'bank_transfer' | 'pay_on_delivery')}>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                >
                  <div className="flex items-center gap-3 mb-3">
                    <RadioGroupItem value="pay_on_delivery" id="pay_on_delivery" disabled={!payOnDeliveryEnabled} />
                    <Truck className="h-5 w-5" />
                    <Label htmlFor="pay_on_delivery" className={`font-semibold ${!payOnDeliveryEnabled ? 'opacity-60' : 'cursor-pointer'}`}>
                      Pay on Delivery
                    </Label>
                  </div>
                  {loadingDetails ? (
