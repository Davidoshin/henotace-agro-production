...
export default function SecurityCentre({ canCreateTeams = true, isSOCDashboard = false }: SecurityCentreProps) {
  ...
  useEffect(() => {
    if (activeTab === 'team') {
      ...
      loadSOCDashboard(); // Load dashboard data for vulnerability stats
    }
  }, [activeTab]);
  ...
  const createTeamMember = async () => {
    try {
      // Convert role_id to integer if it's a string
      const memberData = {
        ...newMember,
        role_id: parseInt(newMember.role_id) || newMember.role_id
      };
      
      console.log('Creating team member with data:', memberData);
      const data = await apiPost('security/team/create/', memberData) as any;
      console.log('API Response:', data);
      ...
      console.error('Team member creation error:', e);
      console.error('Error details:', {
        message: e?.message,
        status: e?.status,
        data: e?.data,
      ...
      const errorMessage = e?.response?.data?.message || 
                          e?.data?.message || 
                          e?.message || 
                          'An error occurred while creating team member';
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage security team and monitor security operations</p>
        </div>
      ...
      <div className="border rounded-lg p-6 space-y-6">
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="dashboard" className="space-y-6">
            {socDashboard ? (
              <>
                {/* Overview Stats */}
                <Card className="mx-6 my-6 border-2 border-dashed border-primary/30 bg-background shadow-lg">
                  <CardHeader className="pb-4 bg-muted/20 border-b border-dashed border-primary/20">
                    <CardTitle className="text-lg font-medium text-center">Security Overview</CardTitle>
                  </CardHeader>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
                      <Card className="border border-border bg-card shadow-sm hover:shadow-md transition-all hover:scale-[1.02]">
                        ...
                        <CardContent className="pt-0">
                          <div className="text-2xl font-bold">{socDashboard.overview.total_alerts}</div>
                      ...
                      <Card 
                        className="border border-border bg-card shadow-sm cursor-pointer hover:shadow-md hover:border-primary/50 transition-all hover:scale-[1.02]"
                      ...
                      >
                        ...
                        <CardContent className="pt-0">
                          <div className="text-2xl font-bold">{socDashboard.overview.active_incidents}</div>
                          <p className="text-xs text-muted-foreground mt-1">
                      ...
                      <Card className="border border-border bg-card shadow-sm hover:shadow-md transition-all hover:scale-[1.02]">
                        ...
                        <CardContent className="pt-0">
                          <div className="text-2xl font-bold">{socDashboard.overview.security_score}%</div>
                      ...
                      >
                        ...
                        <CardContent className="pt-0">
                          <div className="text-2xl font-bold">{socDashboard.overview.vulnerabilities_found}</div>
                          <p className="text-xs text-muted-foreground mt-1">
              ...
              <Card className="mx-6 my-6 border-2 border-dashed border-orange-30 bg-background shadow-lg">
                <CardHeader className="pb-4 bg-muted/20 border-b border-dashed border-orange-20">
                  <CardTitle className="text-lg font-medium text-center">Threat Intelligence</CardTitle>
                </CardHeader>
                <CardContent className="px-6 py-6">
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <Card className="border border-border bg-card shadow-sm hover:shadow-md transition-all hover:scale-[1.02]">
                      ...
                      <CardContent className="pt-0">
                        <div className="text-2xl font-bold text-red-600 dark:text-red-400">{socDashboard.threat_intelligence?.active_campaigns || 0}</div>
                    ...
                    <Card className="border border-border bg-card shadow-sm hover:shadow-md transition-all hover:scale-[1.02]">
                      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-3">
                        <CardTitle className="text-sm font-medium">Blocked Malware</CardTitle>
                        <Shield className="h-4 w-4 text-muted-foreground" />
                      </CardHeader>
                      <CardContent className="pt-0">
                        <div className="text-2xl font-bold text-green-600 dark:text-green-400">{socDashboard.threat_intelligence?.blocked_malware || 0}</div>
                    ...
                    <Card className="border border-border bg-card shadow-sm hover:shadow-md transition-all hover:scale-[1.02]">
                      ...
                      <CardContent className="pt-0">
                        <div className="text-2xl font-bold text-yellow-600 dark:text-yellow-400">{socDashboard.threat_intelligence?.suspicious_ips || 0}</div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                      ...
                      {socDashboard.recent_incidents.slice(0, 5).map((incident) => (
                        ...
                        >
                          <div className="flex-1">
                            ...
                            <p className="text-sm text-muted-foreground">{incident.assigned_to}</p>
                          </div>
                ...
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>Latest security alerts from monitoring systems</CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-4">
        ...
        <TabsContent value="team" className="space-y-6">
          <div className="flex items-center justify-between">
            <h2 className="text-lg md:text-2xl font-bold">Security Team Management</h2>
          ...
          <Card>
            <CardHeader>
              <CardTitle>Security Team Members ({teamMembers.length})</CardTitle>
              <CardDescription>Manage security team members and their roles</CardDescription>
            </CardHeader>
            <CardContent>
              {isLoading ? (
            ...
            <CardHeader>
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  ...
                  <CardDescription>View and manage security vulnerabilities</CardDescription>
                </div>
                <div className="flex gap-2">
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  <Card className="border-red-200 bg-red-50">
                    ...
                    <CardContent>
                      <div className="text-2xl font-bold text-red-600">{socDashboard?.overview?.critical_vulnerabilities || 0}</div>
                  ...
                  <Card className="border-orange-200 bg-orange-50">
                    ...
                    <CardContent>
                      ...
                      <p className="text-xs text-orange-600">Address soon</p>
                    </CardContent>
                  </Card>
                  <Card className="border-yellow-200 bg-yellow-50">
                    ...
                    <CardContent>
                      ...
                      <p className="text-xs text-yellow-600">Plan to fix</p>
                    </CardContent>
                  </Card>
                  <Card className="border-blue-200 bg-blue-50">
                    ...
                    <CardContent>
                      <div className="text-2xl font-bold text-blue-600">{socDashboard?.overview?.vulnerabilities_found || 0}</div>
                      <p className="text-xs text-blue-600">All vulnerabilities</p>
                    </CardContent>
                  </Card>
                </div>
                    ...
                    <div className="space-y-3">
                      {[...Array(Math.min(4, socDashboard?.overview?.critical_vulnerabilities || 0))].map((_, index) => (
                        <div key={index} className="flex items-center justify-between p-4 border rounded-lg hover:bg-muted/50 transition-colors">
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-2">
                              <h3 className="font-medium">Critical Vulnerability #{index + 1}</h3>
                              <Badge className="bg-red-600">Critical</Badge>
                            </div>
                            <p className="text-sm text-muted-foreground mb-2">
                              Security vulnerability detected in system component requiring immediate attention
                            </p>
                            <div className="flex items-center gap-4 text-xs text-muted-foreground">
            ...
            <CardHeader>
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  ...
                  <CardDescription>View security-focused error logs and events</CardDescription>
                </div>
                <div className="flex gap-2">
            ...
            <CardContent>
              {isLoading ? (
                ...
                <div className="text-center py-8 text-muted-foreground">
                  ...
                  <p className="text-sm">Security-related events and errors will appear here</p>
                </div>
              ) : (
                <div className="space-y-4">
      ...
      <Dialog open={showIncidentDialog} onOpenChange={setShowIncidentDialog}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <DialogFooter>
            ...
            <Button 
              onClick={() => selectedIncident && updateIncidentStatus(selectedIncident.id, incidentStatus)}
              disabled={!selectedIncident || incidentStatus === selectedIncident.status}
            >
              Update Status
            </Button>
          </DialogFooter>
